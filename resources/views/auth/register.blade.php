...
@section('title', __('Pendaftaran') . ' - Sekolah Cinta Kasih Tzu Chi')
    ...
    <div class="max-w-6xl w-full bg-white rounded-[2rem] shadow-glass overflow-hidden flex flex-col md:flex-row">
        ...
        <div class="hidden md:flex md:w-1/2 bg-primary relative flex-col justify-between p-12 text-white overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <h2 class="text-4xl font-bold leading-tight mb-6">"{{ __('Pendidikan adalah bekal terbaik untuk perjalanan hidup.') }}"</h2>
                <p class="text-green-100 text-lg">{{ __('Bergabunglah dengan komunitas pembelajar yang berdedikasi untuk keunggulan dan kasih sayang.') }}</p>
            </div>
            ...
            <div class="relative z-10">
                <div class="flex items-center space-x-4">
                    ...
                    <p class="text-sm font-medium text-green-100">{{ __('Bergabung dengan 500+ siswa lainnya') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="w-full md:w-1/2 p-8 md:p-12 bg-white relative">
            <div class="text-center mb-10">
                <h2 class="text-3xl font-bold text-gray-900">{{ __('Buat Akun Baru') }}</h2>
                <p class="text-gray-500 mt-2">{{ __('Silakan lengkapi data diri Anda untuk mendaftar.') }}</p>
            </div>
            ...
            <form action="{{ route('register.store') }}" method="POST" class="space-y-6">
                ...
                <div class="space-y-5">
                    <div class="relative group">
                        <input type="text" name="name" id="name" value="{{ old('name') }}" class="peer w-full px-4 py-3 rounded-xl bg-gray-50 border-2 border-gray-100 text-gray-900 focus:outline-none focus:border-primary focus:bg-white transition-all placeholder-transparent" placeholder="{{ __('Nama Lengkap') }}" required>
                        <label for="name" class="absolute left-4 -top-2.5 bg-white px-1 text-sm text-gray-500 transition-all peer-placeholder-shown:top-3.5 peer-placeholder-shown:text-gray-400 peer-focus:-top-2.5 peer-focus:text-primary peer-focus:font-semibold">{{ __('Nama Lengkap') }}</label>
                    </div>

                    <div class="relative group">
                        <input type="email" name="email" id="email" value="{{ old('email') }}" class="peer w-full px-4 py-3 rounded-xl bg-gray-50 border-2 border-gray-100 text-gray-900 focus:outline-none focus:border-primary focus:bg-white transition-all placeholder-transparent" placeholder="{{ __('Email') }}" required>
                        <label for="email" class="absolute left-4 -top-2.5 bg-white px-1 text-sm text-gray-500 transition-all peer-placeholder-shown:top-3.5 peer-placeholder-shown:text-gray-400 peer-focus:-top-2.5 peer-focus:text-primary peer-focus:font-semibold">{{ __('Email') }}</label>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="relative group">
                            <input type="password" name="password" id="password" class="peer w-full px-4 py-3 rounded-xl bg-gray-50 border-2 border-gray-100 text-gray-900 focus:outline-none focus:border-primary focus:bg-white transition-all placeholder-transparent" placeholder="{{ __('Password') }}" required>
                            <label for="password" class="absolute left-4 -top-2.5 bg-white px-1 text-sm text-gray-500 transition-all peer-placeholder-shown:top-3.5 peer-placeholder-shown:text-gray-400 peer-focus:-top-2.5 peer-focus:text-primary peer-focus:font-semibold">{{ __('Password') }}</label>
                        </div>
                        <div class="relative group">
                            <input type="password" name="password_confirmation" id="password_confirmation" class="peer w-full px-4 py-3 rounded-xl bg-gray-50 border-2 border-gray-100 text-gray-900 focus:outline-none focus:border-primary focus:bg-white transition-all placeholder-transparent" placeholder="{{ __('Konfirmasi') }}" required>
                            <label for="password_confirmation" class="absolute left-4 -top-2.5 bg-white px-1 text-sm text-gray-500 transition-all peer-placeholder-shown:top-3.5 peer-placeholder-shown:text-gray-400 peer-focus:-top-2.5 peer-focus:text-primary peer-focus:font-semibold">{{ __('Konfirmasi') }}</label>
                        </div>
                    </div>
                    ...
                    <div class="relative group">
                        <select name="level" id="level" class="peer w-full px-4 py-3 rounded-xl bg-gray-50 border-2 border-gray-100 text-gray-900 focus:outline-none focus:border-primary focus:bg-white transition-all appearance-none" required onchange="toggleMajor()">
                            <option value="" disabled selected>{{ __('Pilih Jenjang Pendidikan') }}</option>
                        ...
                        <label for="level" class="absolute left-4 -top-2.5 bg-white px-1 text-sm text-primary font-semibold">{{ __('Jenjang') }}</label>
                    </div>
                    ...
                    <div class="hidden animate-fade-in-up" id="major-container">
                        <div class="relative group">
                            ...
                            <label for="major" class="absolute left-4 -top-2.5 bg-white px-1 text-sm text-primary font-semibold">{{ __('Jurusan') }}</label>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative w-full h-14 bg-gray-100 rounded-full overflow-hidden shadow-inner flex items-center p-1 cursor-pointer select-none group" id="slider-container">
                    <div class="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <span class="text-gray-400 font-semibold tracking-wide group-hover:text-gray-500 transition-colors" id="slider-text">{{ __('Geser ke kanan untuk verifikasi') }}</span>
                    </div>
                ...
                <p class="text-center text-gray-500 text-sm">
                    {{ __('Sudah punya akun?') }} <a href="{{ route('login') }}" class="text-primary font-bold hover:underline">{{ __('Masuk disini') }}</a>
                </p>
            </form>
        </div>
    </div>
...
<script>
    ...
    function endDrag() {
        ...
        if (currentX >= maxDrag - 10) {
            ...
            sliderText.textContent = "{{ __('Terverifikasi') }}";
